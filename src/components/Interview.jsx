...
const Interview = () => {
  ...
  const startSpeech = () => {
    ...
    r.onspeechend = () => { setAnswer(idx, finalTranscript); };
    r.onerror = (e) => {
      if (e.error === "not-allowed") message.error("Microphone access denied.");
      else if (e.error === "no-speech") message.warning("No speech detected.");
      setListening(false);
    };
    r.onend = () => { setListening(false); setAnswer(idx, finalTranscript); };
    r.start(); recRef.current = r; setListening(true);
  };

  const stopSpeech = () => { if (recRef.current) { recRef.current.stop(); recRef.current = null; } setListening(false); };
  ...
  const onStart = async (e) => {
    ...
    try {
      ...
      setQuestions(res.data.questions); setInterviewId(res.data.interviewId);
      setAnswers(new Array(res.data.questions.length).fill("")); setIdx(0); setStage("questions");
  ...
  const handleNext = () => {
    ...
    stopSpeech(); setIdx(i => i + 1); setShowCode(false); setCode("// Write your code here\n");
  };
  ...
  const reset = () => {
    ...
    setQuestions([]); setAnswers([]); setShowCode(false); setRoleInput("");
  };
  ...
  /* ── Mode picker ── */
  if (!mode) return (
    <div className="page mesh-bg iv-center-page" style={{ minHeight: "100vh" }}>
      ...
      <div style={{ width: "100%", maxWidth: 500, padding: "0 20px", position: "relative", zIndex: 1 }}>
        ...
        <div className="card" style={{ padding: 44, borderRadius: 28, boxShadow: "var(--shadow-lg)" }}>
          <div className="iv-setup-header">
            <div className="iv-setup-icon">{mode === "voice" ? "🎤" : "⌨️"}</div>
            <div>
          ...
          <form onSubmit={onStart} className="iv-setup-form">
              ...
              {showSuggestions && filteredSuggestions.length > 0 && (
                <div className="iv-suggestions">
                  {filteredSuggestions.map(s => (
                    <div key={s} className="iv-suggestion-item" onMouseDown={() => { setRoleInput(s); setShowSuggestions(false); }}>{s}</div>
                  ))}
                </div>
              )}
            </div>
            ...
            <button className="btn btn-primary" type="submit" disabled={loading} style={{ width: "100%", justifyContent: "center", height: 50, fontSize: 15, borderRadius: 13, marginTop: 4 }}>
              {loading
                ? <><span className="spin-indicator" /> Generating questions…</>
                : "Start Interview →"}
            </button>
          </form>
        </div>
      ...
      <div className="page" style={{ background: "var(--bg)" }}>
        <div className="container" style={{ paddingTop: 88, paddingBottom: 60, maxWidth: 820 }}>
          <div className="card" style={{ padding: "44px 36px", textAlign: "center", marginBottom: 20, borderRadius: 28, boxShadow: "var(--shadow-lg)" }}>
            ...
            <div className="iv-results-score-row">
              <span className="g-text iv-results-big-score">{result.overallScore}</span>
              <span style={{ fontSize: 22, color: "var(--muted2)" }}>/ {maxScore}</span>
            </div>
            <div className="iv-results-prog">
          ...
          {dd && (
            <div className="iv-results-deepdive">
              {dd.strengths?.length > 0 && (
                <div className="card card-p" style={{ borderColor: "rgba(16,185,129,0.3)", background: "rgba(16,185,129,0.04)" }}>
                  ...
                  {dd.strengths.map((s,i) => <p key={i} className="iv-dd-list-item"><span style={{ color: "#10b981" }}>•</span>{s}</p>)}
                </div>
              )}
              {dd.areasToImprove?.length > 0 && (
                <div className="card card-p" style={{ borderColor: "rgba(239,68,68,0.3)", background: "rgba(239,68,68,0.04)" }}>
                  ...
                  {dd.areasToImprove.map((s,i) => <p key={i} className="iv-dd-list-item"><span style={{ color: "#ef4444" }}>•</span>{s}</p>)}
                </div>
              )}
          ...
          {result.questions?.map((q, i) => (
            <div key={i} className="card card-p" style={{ marginBottom: 12, borderColor: `${scoreColor(q.score)}40` }}>
              ...
              <div className="iv-q-answer-box"><strong>Answer: </strong>{q.userAnswer || "—"}</div>
              ...
              {result.deepDiveFeedback?.improvedAnswers?.find(ia => ia.question === q.question) && (
                <div className="iv-improved-box">
                  <span className="iv-improved-label">Improved Answer</span>
  ...
  return (
    <div className="page" style={{ background: "var(--bg)", display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh", padding: "80px 16px 40px" }}>
      <div style={{ width: "100%", maxWidth: 700 }}>
        ...
        <div className="card iv-question-card" style={{ padding: "36px 38px", borderRadius: 24, boxShadow: "var(--shadow-lg)" }}>
          {thinking ? (
            <div className="iv-thinking">
              <div className="iv-thinking-icon">🤖</div>
            ...
            {mode === "voice" ? (
              <div className="iv-voice-section">
                <div className="iv-voice-center">
                  ...
                  {listening && <p style={{ fontSize: 11, color: "var(--muted)", margin: "4px 0 0" }}>Speak clearly • Chrome works best</p>}
                </div>
                <div>
                  ...
                  <textarea
                    ...
                    value={answers[idx]} onChange={e => setAnswer(idx, e.target.value)}
